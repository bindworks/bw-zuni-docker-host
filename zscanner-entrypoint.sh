...
while NEXT_SLEEP=$(( $NEXT_SLEEP > $MAX_SLEEP ? $MAX_SLEEP : $NEXT_SLEEP )); 
    ...
    EXITCODE=0
    java -jar "$ZSCANNER_JAR_DIR/zscanner.jar" "-Dzscanner.update.enable=true" "-Dzscanner.update.jarUrl=$ZSCANNER_JAR_URL" "-Dzscanner.update.jarSha256=$CURRENT_VERSION_CHECKSUM" "$@" || EXITCODE=$?
